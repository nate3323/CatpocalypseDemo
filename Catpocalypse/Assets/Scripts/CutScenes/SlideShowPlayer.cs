...
public partial class SlideShowPlayer : MonoBehaviour
{
    ...
    private bool _goToNextSlide;
    private bool _triggerSlideChange;
    ...
    {
        // Tell the slide show to stop playing.
        _stopReason = StopReasons.Stopped;
    }

    private void OnTriggerSlideChange(object sender, EventArgs e)
    {
        _triggerSlideChange = true;
    }
    ...
    private IEnumerator PresentSlideShow()
    {
        ...
        while (true)
        {
            ...
            {
                // NOTE: This variable can also be set to true by the OnTriggerSlideChange() event handler at the top of this class.
                _triggerSlideChange = true;
            }

            // Check if it is time to switch to the next slide.
            if (_triggerSlideChange)
            {
                _triggerSlideChange = false;

                _currentSlideShowStateInfo.MoveToNextSlide();
                slideDuration = _currentSlideShowStateInfo.GetCurrentSlideDisplayTime();

                if (_currentSlideShowStateInfo.CurrentSlideIndex >= _currentlyPlayingSlideShow.SlideCount - 1)
                    _slideShowUI.ShowContinueButton();
            ...
            if (!_slideShowUI.IsTransitioning &&
                ((_currentlyPlayingSlideShow.AdvancementMode != SlideShow.AdvancementModes.WhenSlideDurationExpires && _goToNextSlide) ||
                 (_currentlyPlayingSlideShow.AdvancementMode != SlideShow.AdvancementModes.WhenAnyKeyPressed && _currentSlideShowStateInfo.CurrentSlideElapsedTime >= _currentSlideShowStateInfo.GetCurrentSlideTransitionTime())))
            {
                if (_currentSlideShowStateInfo.CurrentSlideIndex >= _currentlyPlayingSlideShow.SlideCount - 1)
                {
    ...
    {
        if (!_currentlyPlayingSlideShow.FadeSlideShowInAndOut)
        {
        ...
        float fadeInTime = _currentlyPlayingSlideShow.DefaultFadeInTime;
    ...
    private IEnumerator FadeOutSlideShow()
    {
        ...
        float fadeOutTime = _currentlyPlayingSlideShow.DefaultFadeOutTime;
        yield return StartCoroutine(_slideShowUI.DoFade(Slide.FadeType.FadeOut, 
                                                        fadeOutTime));
